...
# Use a real PostgreSQL test database (same credentials, "_test" suffix on DB name)
# Requires: createdb <DB_NAME>_test
TEST_DATABASE_URL = "postgresql+asyncpg://user:pass@localhost/mydb_test"
...
async def test_query_user(session, test_user):
    """Test user query."""
...
async def test_update_user(session, test_user):
    """Test user update."""
...
8. **Testing**: Use a PostgreSQL test database (matching production) with transactional rollback per test
